...
handle_info({node, Node}, #pro_wx_state{holder = Holder} = State) ->
    State2 = State#pro_wx_state{node = Node},
    change_node(Node),
    refresh_grid(Holder),
    {noreply, State2};
...
get_row(From, Row, all, Info, {Sort, Dir}) ->
    ...
    From ! {self(), lists:flatten(io_lib:format("~p", [ProcInfo]))};
